...
describe("RoomListStoreV3", () => {
    ...
    describe("Updates", () => {
        it("Room is re-inserted on timeline event", async () => {
            ...
            const room = rooms[37];
            const event = mkMessage({ room: room.roomId, user: `@foo${3}:nobody.network`, ts: 1000, event: true });
        ...
        it("Predecessor room is removed on room upgrade", async () => {
            ...
            // Let's say that !foo32:nobody.network is being upgraded
            const oldRoom = rooms[32];
            // Create a new room with a predecessor event that points to oldRoom
            const newRoom = new Room("!foonew:nobody.network", client, client.getSafeUserId(), {});
        ...
        it("Rooms are re-inserted on m.direct event", async () => {
            ...
            const content = {
                "@bar1:nobody.network": ["!foo1:nobody.network", "!foo2:nobody.network"],
                "@bar2:nobody.network": ["!foo3:nobody.network", "!foo4:nobody.network"],
                "@bar3:nobody.network": ["!foo5:nobody.network"],
            };
            const event = mkEvent({
                event: true,
                content,
                user: "@foo:nobody.network",
            ...
            dispatcher.dispatch(
                {
                    action: "MatrixActions.Event.decrypted",
                    event: {
                        getRoomId: () => "!doesnotexist:nobody.network",
                        getId: () => "some-id",
                    },
                },
                true,
        ...
        describe("Update from read receipt", () => {
            ...
            it("Read receipt from other users do not cause room to be re-inserted", async () => {
                const { store, rooms, dispatcher } = await getRoomListStore();
                const event = getReadReceiptEvent("@foobar:nobody.network");
                const fn = jest.fn();
                store.on(LISTS_UPDATE_EVENT, fn);
                dispatcher.dispatch(
                    {
        ...
        function createSpace(rooms: Room[], inSpaceIndices: number[], client: MatrixClient) {
            ...
            const spaceRoom = mkSpace(client, "!space1:nobody.network", [], roomIds);
        ...
        describe("Filters", () => {
            ...
            it("supports filtering by people and rooms", async () => {
                ...
                jest.spyOn(DMRoomMap, "shared").mockImplementation((() => {
                    return {
                        getUserIdForRoomId: (id) => (ids.includes(id) ? "@myuser:nobody.network" : ""),
                    };
                }) as () => DMRoomMap);
